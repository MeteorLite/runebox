...
package io.runebox.asm.tools;

import java.io.*;
import java.util.*;

import io.runebox.asm.benchmark.*;
...
import io.runebox.asm.optimize.*;
...
public class BloatBenchmark {
	...
	private static void intraBloat(final Collection liveMethods,
		...
		for (int count = 0; iter.hasNext(); count++) {
			...
			try {
				io.runebox.asm.optimize.Main.TRACE = BloatBenchmark.TRACE;
				if (!BloatBenchmark.VERIFY) {
					io.runebox.asm.optimize.Main.VERIFY = false;
				}
				io.runebox.asm.optimize.Main.bloatMethod(live, context);
